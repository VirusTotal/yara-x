...
//!
//! The browser and `wasm32-wasip1` backends both expose a small, Wasmtime-like
//! API to the rest of the crate. This module contains the backend-agnostic
//! pieces of that shim, while each concrete runtime implements the
...
pub struct Store<T, B: RuntimeBackend> {
...
pub struct Linker<T, B: RuntimeBackend> {
...
pub struct Module<B: RuntimeBackend> {
...
/// Instantiated module.
pub struct Instance<B: RuntimeBackend> {
...
pub struct TypedFunc<P, R, B: RuntimeBackend> {
...
/// Converts a raw trampoline value into the ABI representation expected by the
/// backend.
