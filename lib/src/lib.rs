...
pub use models::Rule;
pub use modules::YaraModule;
pub use modules::YaraModuleDescriptorFn;
pub use modules::YaraModuleMainFn;
...
///
/// External crates that register [`YaraModule`]s can use this re-export to
/// guarantee they link against the exact same version of `inventory` as
/// `yara-x`.
pub use inventory;

/// Opaque scan context passed as first argument to functions exported from a
/// [`YaraModule`] via `#[yara_x::module_export(yara_x_crate = "yara_x")]`.
///
...
/// Attribute macro for exporting a callable function from a [`YaraModule`].
///
...
// Not available on the `wasm32` target (wasmtime is not a dependency there).
#[cfg(not(target_family = "wasm"))]
#[doc(hidden)]
