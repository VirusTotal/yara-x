...
/// Describes a rule that was ignored during compilation and the reason why.
...
impl IgnoredRule {
    /// Creates a new [`IgnoredRule`].
    pub fn new(
        rule_name: impl Into<String>,
        reason: IgnoredRuleReason,
    ) -> Self {
        Self { rule_name: rule_name.into(), reason }
    }

    /// Returns the name of the ignored rule.
    pub fn rule_name(&self) -> &str {
        &self.rule_name
    }

    /// Returns the reason why the rule was ignored.
    pub fn reason(&self) -> &IgnoredRuleReason {
        &self.reason
    }
}
...
pub enum IgnoredRuleReason {
    ...
    IgnoredRule(String),
    /// The rule was ignored because of a compilation error. Contains the
...
pub struct Compiler<'a> {
    ...
    /// Vector containing the rules that were ignored during compilation,
    /// along with the reason why they were ignored.
    ignored_rules: Vec<IgnoredRule>,
...
impl<'a> Compiler<'a> {
    ...
    /// Returns the rules that were ignored during compilation, along with the
    /// reason why they were ignored.
    #[inline]
    pub fn ignored_rules(&self) -> &[IgnoredRule] {
        self.ignored_rules.as_slice()
    }
            ...
            match item {
                ...
                ast::Item::Rule(rule) => {
                    if let Err(err) = self.c_rule(rule) {
                        self.ignored_rules.push(IgnoredRule::new(
                            rule.identifier.name,
                            IgnoredRuleReason::CompileError(err.clone()),
                        ));
                        self.errors.push(err);
                    }
                }
