...
pub struct Compiler<'a> {
    ...
    /// Linter errors generated while compiling the rules. These are kept
    /// separate from compiler errors for reporting purposes. We want to be able
    /// to report multiple linter errors in a single run, rather than just the
    /// first error like normal compiler errors.
    linter_errors: Vec<CompileError>,
...
impl<'a> Compiler<'a> {
    ...
    /// Retrieves all linter errors generated by the compiler.
    ///
    /// This method returns every linter error encountered during the
    /// compilation, across all invocations of [`Compiler::add_source`].
    #[inline]
    pub fn linter_errors(&self) -> &[CompileError] {
        self.linter_errors.as_slice()
    }
