...
impl AuthenticodeHasher for MsiHasher<'_> {
    fn hash(&self, digest: &mut dyn digest::Update) -> Option<()> {
        ...
        let header_bytes = &self.data[..sector_size];
...
impl Msi {
    pub fn parse(data: &[u8]) -> Result<MsiProto, &'static str> {
        let olecf = OLECF::parse(data)?;
        ...
        let (sig_name, _) = sig_stream.ok_or("No digital signature stream found")?;
        let sig_data = olecf.get_stream_data(sig_name).map_err(|_| "Failed to read digital signature stream")?;

        let hasher = MsiHasher { data, olecf: &olecf };
        let signatures = AuthenticodeParser::parse(&sig_data, &hasher)
            .map_err(|_| "Failed to parse signature")?;

        let mut msi_proto = MsiProto::new();
        let pb_signatures: Vec<_> = signatures
            .iter()
            .map(protos::pe::Signature::from)
            .collect();
