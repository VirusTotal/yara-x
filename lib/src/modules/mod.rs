...
pub(crate) struct Module {
    ...
    pub main_fn: Option<YaraModuleMainFn>,
...
/// Signature of the main function of a [`YaraModule`] registered by an
/// external crate.
///
...
pub type YaraModuleDescriptorFn = fn() -> MessageDescriptor;

/// Description of a YARA module supplied by an external crate.
///
/// `YaraModule` is the public counterpart of the (private) [`Module`]
...
pub struct YaraModule {
    ...
    pub root_descriptor: YaraModuleDescriptorFn,
...
inventory::collect!(YaraModule);

/// Global registry of all YARA modules known to the current binary.
///
/// Both built-in modules (registered via `inventory::submit!` in each module's
/// own source file) and external modules (registered by downstream crates via
/// `yara_x::inventory::submit!`) are collected here through
/// [`inventory::iter::<YaraModule>()`]. If two modules share the same name,
/// the first one encountered at link time takes precedence.
pub(crate) static REGISTERED_MODULES: LazyLock<
    FxHashMap<&'static str, Module>,
> = LazyLock::new(|| {
    let mut modules = FxHashMap::default();
    for m in inventory::iter::<YaraModule>() {
        modules.entry(m.name).or_insert_with(|| Module {
            main_fn: m.main_fn,
            rust_module_name: m.rust_module_name,
            root_struct_descriptor: (m.root_descriptor)(),
        });
    }
    modules
});
...
pub mod mods {
    ...
    */
    ...
    pub fn invoke_with_meta_dyn<T: protobuf::MessageFull>(
        ...
        let (_, module) =
            super::REGISTERED_MODULES.iter().find(|(_, module)| {
    ...
    pub fn module_names() -> impl Iterator<Item = &'static str> {
        ...
        super::REGISTERED_MODULES.keys().sorted_by_key(|k| **k).copied()
    }
