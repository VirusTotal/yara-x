...
fn zip_file_extraction_scan() {
    ...
    let results = scanner.scan(&zip_data).expect("scan should not fail");
...
fn test_zip_module_rules() {
    let rules = crate::compile(
        ...
        rule suspicious_zip {
            condition:
                zip.is_zip and
                for any entry in zip.entries: (
                    entry.filename == "suspicious_payload.exe" and
                    entry.compression == zip.DEFLATED
                )
        }
        "#,
    )
