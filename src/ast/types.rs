...
/// Type of a YARA expression or identifier in the AST.
...
impl From<&RuntimeFieldType> for Type {
    fn from(ty: &RuntimeFieldType) -> Self {
        match ty {
...
/// Value of a YARA expression or identifier, if it can be known at parse time.
#[derive(Clone)]
pub enum Value {
    Unknown,
    Integer(i64),
