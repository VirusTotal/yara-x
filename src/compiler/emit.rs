...
/// Emits code for caching exceptions caused by undefined values.
///
...
/// the top of the stack. However, `expr` can make use of `raise_exception` for
...
/// at the point where `raise_exception` was found, and the control transferred
...
/// it was interrupted by `raise_exception`.
///
...
///    I32,
///    |try_block| {
///       raise_exception(ctx, try_block, I32);   // The exception is raised here ...
...
pub(super) fn try_except(
    ...
    // Create a new block containing `expr`. When an exception
    // is raised from within `expr` the control flow will jump
    // out of this block via a `br` instruction.
    instr.block(ty, |block| {
        // Push the ID of the current block in the handlers stack.
        ctx.borrow_mut().exception_handler_stack.push(block.id());
        expr(block);
    });
...
pub(super) fn raise(
    ...
    // Put in the stack a dummy result for the code block where `raise`
    // is located. In WebAssembly each block has a return type, and the
    // top of the stack must be of that type when leaving the block, both
    // because the end of the block was reached or, as in this case,
    // because a `br` instruction jumped out the block.
    match ty {
