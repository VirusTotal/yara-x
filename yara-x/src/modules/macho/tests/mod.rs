...
mod tests {
    ...
    #[test]
    fn test_constants_in_rules() {
        let mut compiler = crate::compiler::Compiler::new();

        compiler
            .add_source(
                r#"import "macho"
                rule rule_magic { condition: macho.MH_MAGIC == 0xfeedface }
                rule rule_file_flag_dylibs { condition: macho.MH_NO_REEXPORTED_DYLIBS == 0x00100000 }
                rule rule_cigam { condition: macho.MH_CIGAM == 0xcefaedfe }
                rule rule_cpu_mips { condition: macho.CPU_TYPE_MIPS == 0x00000008 }
                "#,
            )
            .unwrap();
        ...
        assert_eq!(scanner.scan(&[]).unwrap().matching_rules().len(), 4);
    }

    #[test]
    fn test_invalid_constants_in_rules() {
        let mut compiler = crate::compiler::Compiler::new();

        compiler
            .add_source(
                r#"import "macho"
                rule rule_magic { condition: macho.MH_MAGIC == 0xfeeeeeee }
                rule rule_file_flag_dylibs { condition: macho.MH_NO_REEXPORTED_DYLIBS == 0x99999999 }
                rule rule_cigam { condition: macho.MH_CIGAM == 0xaaaaaaaa }
                rule rule_cpu_mips { condition: macho.CPU_TYPE_MIPS == 0x00000000 }
                "#,
            )
            .unwrap();
    ...
    #[test]
    fn test_file_index_for_arch_type() {
        // Initialize minimal mock of input data that is needed
        let macho_file = [
            // Fat header
        ...
        // Define a rule that checks file index for our mock architecture.
        let rule_src = r#"
        import "macho"
        rule check_entry_point_arch_type {
            condition: macho.file_index_for_arch(0x00000007) == 0
        }
        "#;

        let rules = crate::compile(rule_src).unwrap();
        ...
        assert_eq!(num_matching_rules, 1, "Expected one rule to match");
    }

    #[test]
    fn test_file_index_for_arch_subtype() {
        // Initialize the main parts of the data
        let macho_file = [
            // Fat header
            0xCA, 0xFE, 0xBA, 0xBE, // magic
            0x00, 0x00, 0x00, 0x01, // nfat_arch (2 architectures)
        ...
        let rule_src = r#"
        import "macho"
        rule check_entry_point_arch_subtype {
            condition: macho.file_index_for_arch(0x00000007, 0x00000003) == 0
        }

        rule check_entry_point_arch_subtype_failing {
            condition: macho.file_index_for_arch(0x00000007, 0x00000008) == 0
        }
        "#;
    ...
    #[test]
    fn test_entry_point_for_arch_type() {
        let rule_src = r#"
        import "macho"
        rule check_entry_point_arch_subtype {
            condition: not defined macho.entry_point_for_arch(0x00000007)
        }
        ...
        assert_eq!(
            ...
            "Test should succeed also for an empty input"
        );
    }

    #[test]
    fn test_entry_point_for_arch_subtype() {
        let rule_src = r#"
        import "macho"
        rule check_entry_point_arch_subtype {
            condition: not defined macho.entry_point_for_arch(0x00000007, 0x00000003)
        }
