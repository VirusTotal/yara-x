...
const jsEntrypoint = path.join(pkgDir, "yara_wasm.js");
const wasmPath = path.join(pkgDir, "yara_wasm_bg.wasm");
const bundleWasmPath = path.join(distDir, "yara_wasm_bundle_bg.wasm");
...
test("validateRules accepts a single rule string", () => {
  const rule = `
    rule single_ok {
      strings:
        $a = "abc"
...
test("validateRules accepts the full browser module set", () => {
  const moduleRules = [
    "console",
    "crx",
    "dex",
  ...
  const result = validateRules(moduleRules);
  assert.equal(result.valid, true, result.errors.join("\n"));
  assert.deepEqual(result.errors, []);
});

test("scanRules supports the browser-backed time module end to end", () => {
...
test("validateRules accepts a list of rules", () => {
  const rules = [
...
test("validateRules returns syntax diagnostics without throwing", () => {
  const badRule = 'rule broken { strings: $a = "x" condition: and }';

  const result = validateRules(badRule);
  assert.equal(result.valid, false);
  assert.ok(result.errors.length > 0);
});

test("validateRules throws on invalid rules payload type", () => {
  assert.throws(
    () => validateRules({ not: "a rule string" }),
    /rules must be a string or array of strings/,
  );
});

test("validateRules throws on empty rules array", () => {
  assert.throws(() => validateRules([]), /at least one rule must be provided/);
});

test("validateRules reports a sensible error for include directives", () => {
  const includeRule = `
    include "missing.yar"
    rule include_test {
      condition:
  ...
  const result = validateRules(includeRule);
  assert.equal(result.valid, false);
  assert.ok(result.errors.length > 0);
  assert.match(result.errors[0], /error including file/i);
  assert.match(result.errors[0], /(not supported|failed)/i);
});

test("scanRules returns no matches when nothing matches", () => {
  const rule = `
    rule no_match {
      strings:
        $a = "abc"
  ...
  const result = scanRules(rule, new Uint8Array([0x78, 0x79, 0x7a]));
  assert.equal(result.valid, true);
  assert.equal(result.matches.length, 0);
});

test("scanRules returns compile errors for invalid rules", () => {
  const badRule = "rule invalid { condition: and }";

  const result = scanRules(badRule, new Uint8Array([0x61]));
  assert.equal(result.valid, false);
  assert.ok(result.errors.length > 0);
  assert.equal(result.matches.length, 0);
});

test("scanRules reports include loading failure without throwing", () => {
  const includeRule = `
    include "missing.yar"
    rule include_scan_test {
      condition:
        true
    }
  `;

  const result = scanRules(includeRule, new Uint8Array([0x61]));
  assert.equal(result.valid, false);
  assert.equal(result.matches.length, 0);
  assert.ok(result.errors.length > 0);
  assert.match(result.errors[0], /error including file/i);
...
test("scanRules supports list-of-rules input and returns only matching rules", () => {
  const rules = [
    `
      rule should_match {
        strings:
  ...
  const result = scanRules(rules, new Uint8Array([0x61, 0x62, 0x63]));
  assert.equal(result.valid, true);
  assert.equal(result.matches.length, 1);
  assert.equal(result.matches[0].identifier, "should_match");
});

test("scanRules returns multiple matches from multiple rules in a single source", () => {
  const rules = `
    rule hit_one {
      strings:
        $a = "abc"
  ...
  const result = scanRules(rules, payload);
...
test("scanRules returns multiple matches from an array of rule sources", () => {
  const rules = [
    `
      rule arr_hit_one {
        strings:
...
test("scanRules returns match details, tags, and metadata values", () => {
  const rules = `
...
test("scanRules throws when rules payload type is invalid", () => {
  assert.throws(
    () => scanRules({ invalid: true }, new Uint8Array([0x61])),
    /rules must be a string or array of strings/,
  );
});
...
test("hash module functions work in rule conditions", () => {
  ...
  const result = scanRules(rules, new TextEncoder().encode("foobarbaz"));
  assert.equal(result.valid, true);
...
test("math module functions work in rule conditions", () => {
  ...
  const result = scanRules(rules, new TextEncoder().encode("AAAAB"));
  assert.equal(result.valid, true);
...
test("string module functions work in rule conditions", () => {
  ...
  const result = scanRules(rules, new Uint8Array([0x00]));
  assert.equal(result.valid, true);
...
test("scanRules stays correct across repeated compile+scan rounds", () => {
  const rounds = 40;
...
test("Compiler exposes errors/warnings and is consumed by build()", () => {
  const compiler = new Compiler();
  assert.throws(
