...
pub fn validate_rules(rule_sources: &[String]) -> ValidationResult {
    let compile_outcome = compile_rules(rule_sources);
    ValidationResult {
        valid: compile_outcome.errors.is_empty(),
        errors: compile_outcome.errors,
        warnings: compile_outcome.warnings,
    }
}

pub fn scan_rules(rule_sources: &[String], payload: &[u8]) -> ScanResult {
    let compile_outcome = compile_rules(rule_sources);
...
pub fn validate_rules_js(rules: JsValue) -> Result<JsValue, JsValue> {
    let rule_sources = parse_rule_sources_js(rules)?;
    let result = validate_rules(&rule_sources);
    serde_wasm_bindgen::to_value(&result)
        .map_err(|err| js_error(format!("failed to serialize result: {err}")))
}

#[cfg(target_family = "wasm")]
#[wasm_bindgen(js_name = scanRules)]
pub fn scan_rules_js(
    rules: JsValue,
    payload: &[u8],
) -> Result<JsValue, JsValue> {
    let rule_sources = parse_rule_sources_js(rules)?;
    let result = scan_rules(&rule_sources, payload);
    serde_wasm_bindgen::to_value(&result)
        .map_err(|err| js_error(format!("failed to serialize result: {err}")))
}
...
#[cfg(target_family = "wasm")]
fn parse_rule_sources(rules: RuleInput) -> Result<Vec<String>, String> {
    let rules = match rules {
        RuleInput::One(rule) => vec![rule],
        RuleInput::Many(rules) => rules,
    };

    if rules.is_empty() {
        return Err("at least one rule must be provided".to_owned());
    }

    if rules.iter().all(|rule| rule.trim().is_empty()) {
        return Err("at least one rule must be non-empty".to_owned());
    }
...
#[cfg(target_family = "wasm")]
fn parse_rule_sources_js(value: JsValue) -> Result<Vec<String>, JsValue> {
    let parsed: RuleInput =
        serde_wasm_bindgen::from_value(value).map_err(|err| {
            js_error(format!(
                "rules must be a string or array of strings: {err}"
            ))
        })?;

    parse_rule_sources(parsed).map_err(js_error)
}
...
mod tests {
    ...
    fn reports_syntax_errors() {
        ...
        let result = validate_rules(&[invalid_rule]);
