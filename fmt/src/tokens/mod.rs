...
use yara_x_parser_ng::cst::{Event, SyntaxKind};
use yara_x_parser_ng::Events;
...
impl<'src> Tokens<'src> {
    pub fn new(source: &'src str, events: Events<'src>) -> Self {
        ...
        loop {
            match self.events.next()? {
                ...
                Event::Token { kind, span } => {
                    let token_str = &self.source[span.range()];
